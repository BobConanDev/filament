...
using ResourceManager = fvkmemory::ResourceManager;
...
VulkanDriver::VulkanDriver(VulkanPlatform* platform, VulkanContext const& context,
    ...
    : mPlatform(platform),
      ...
      mCommands(mPlatform->getDevice(), mPlatform->getGraphicsQueue(),
              mPlatform->getGraphicsQueueFamilyIndex(), &mContext),
      ...
      mDescriptorSetManager(mPlatform->getDevice()),
    ...
    ResourceManager::init(driverConfig.handleArenaSize,
...
void VulkanDriver::updateDescriptorSetBuffer(
        ...
        uint32_t offset,
        uint32_t size) {
    auto set = resource_ptr<VulkanDescriptorSet>::cast(dsh);
    auto buffer = resource_ptr<VulkanBufferObject>::cast(boh);
...
void VulkanDriver::updateDescriptorSetTexture(
    ...
    auto set = resource_ptr<VulkanDescriptorSet>::cast(dsh);
    auto texture = resource_ptr<VulkanTexture>::cast(th);
...
void VulkanDriver::createRenderPrimitiveR(Handle<HwRenderPrimitive> rph,
        ...
        PrimitiveType pt) {
    auto ptr = resource_ptr<VulkanRenderPrimitive>::make(rph, pt, vbh, ibh);
    ptr.inc();
}
...
void VulkanDriver::destroyRenderPrimitive(Handle<HwRenderPrimitive> rph) {
    ...
    auto ptr = resource_ptr<VulkanRenderPrimitive>::cast(rph);
    ptr.dec();
}

void VulkanDriver::createVertexBufferInfoR(Handle<HwVertexBufferInfo> vbih, uint8_t bufferCount,
        uint8_t attributeCount, AttributeArray attributes) {
    auto vbi = resource_ptr<VulkanVertexBufferInfo>::make(vbih, bufferCount, attributeCount,
            attributes);
    vbi.inc();
}
...
void VulkanDriver::destroyVertexBufferInfo(Handle<HwVertexBufferInfo> vbih) {
    ...
    auto vbi = resource_ptr<VulkanVertexBufferInfo>::cast(vbih);
    vbi.dec();
}

void VulkanDriver::createVertexBufferR(Handle<HwVertexBuffer> vbh, uint32_t vertexCount,
        Handle<HwVertexBufferInfo> vbih) {
    auto vb = resource_ptr<VulkanVertexBuffer>::make(vbh, mContext, mStagePool, vertexCount, vbih);
    vb.inc();
}
...
void VulkanDriver::destroyVertexBuffer(Handle<HwVertexBuffer> vbh) {
    ...
    auto vb = resource_ptr<VulkanVertexBuffer>::cast(vbh);
    vb.dec();
}
...
void VulkanDriver::createIndexBufferR(Handle<HwIndexBuffer> ibh, ElementType elementType,
    ...
    auto ib = resource_ptr<VulkanIndexBuffer>::make(ibh, mAllocator, mStagePool, elementSize,
            indexCount);
    ib.inc();
}
...
void VulkanDriver::destroyIndexBuffer(Handle<HwIndexBuffer> ibh) {
    ...
    auto ib = resource_ptr<VulkanIndexBuffer>::cast(ibh);
    ib.dec();
}

void VulkanDriver::createBufferObjectR(Handle<HwBufferObject> boh, uint32_t byteCount,
        BufferObjectBinding bindingType, BufferUsage usage) {
    auto bo = resource_ptr<VulkanBufferObject>::make(boh, mAllocator,
...
void VulkanDriver::destroyBufferObject(Handle<HwBufferObject> boh) {
    ...
    auto bo = resource_ptr<VulkanBufferObject>::cast(boh);
    bo.dec();
}
...
void VulkanDriver::createTextureR(Handle<HwTexture> th, SamplerType target, uint8_t levels,
        ...
        TextureUsage usage) {
    auto texture = resource_ptr<VulkanTexture>::make(th, mPlatform->getDevice(),
...
void VulkanDriver::createTextureViewR(Handle<HwTexture> th, Handle<HwTexture> srch,
    ...
    auto src = resource_ptr<VulkanTexture>::cast(srch);
    auto texture = resource_ptr<VulkanTexture>::make(th, mPlatform->getDevice(),
...
void VulkanDriver::createTextureViewSwizzleR(Handle<HwTexture> th, Handle<HwTexture> srch,
   ...
   auto src = resource_ptr<VulkanTexture>::cast(srch);
   auto texture = resource_ptr<VulkanTexture>::make(th, mPlatform->getDevice(),
           mPlatform->getPhysicalDevice(), mContext, mAllocator, &mCommands,
           src, swizzle);
   texture.inc();
}
...
void VulkanDriver::createProgramR(Handle<HwProgram> ph, Program&& program) {
    auto vprogram = resource_ptr<VulkanProgram>::make(ph, mPlatform->getDevice(), program);
    vprogram.inc();
}
...
void VulkanDriver::destroyProgram(Handle<HwProgram> ph) {
    ...
    auto vprogram = resource_ptr<VulkanProgram>::cast(ph);
    vprogram.dec();
}

void VulkanDriver::createDefaultRenderTargetR(Handle<HwRenderTarget> rth, int) {
    assert_invariant(!mDefaultRenderTarget);
    auto renderTarget = resource_ptr<VulkanRenderTarget>::make(rth);
...
void VulkanDriver::createRenderTargetR(Handle<HwRenderTarget> rth,
    ...
    for (int i = 0; i < MRT::MAX_SUPPORTED_RENDER_TARGET_COUNT; i++) {
        if (color[i].handle) {
            colorTargets[i] = {
                .texture = resource_ptr<VulkanTexture>::cast(color[i].handle),
                .level = color[i].level,
    ...
    if (depth.handle) {
        depthStencil[0] = {
            .texture = resource_ptr<VulkanTexture>::cast(depth.handle),
            .level = depth.level,
            .layerCount = layerCount,
    ...
    if (stencil.handle) {
        depthStencil[1] = {
            .texture = resource_ptr<VulkanTexture>::cast(stencil.handle),
            .level = stencil.level,
            .layerCount = layerCount,
    ...
    auto rt = resource_ptr<VulkanRenderTarget>::make(rth, mPlatform->getDevice(),
...
void VulkanDriver::destroyRenderTarget(Handle<HwRenderTarget> rth) {
    ...
    auto rt = resource_ptr<VulkanRenderTarget>::cast(rth);
...
void VulkanDriver::createFenceR(Handle<HwFence> fh, int) {
    ...
    // buffer.
    auto fence = resource_ptr<VulkanFence>::cast(fh);
    fence->fence = commandBuffer.fence;
}
...
void VulkanDriver::createSwapChainR(Handle<HwSwapChain> sch, void* nativeWindow, uint64_t flags) {
    ...
    auto swapChain = resource_ptr<VulkanSwapChain>::make(sch, mPlatform, mContext, mAllocator,
            &mCommands, mStagePool, nativeWindow, flags);
    swapChain.inc();
}
...
void VulkanDriver::createSwapChainHeadlessR(Handle<HwSwapChain> sch, uint32_t width,
    ...
    auto swapChain = resource_ptr<VulkanSwapChain>::make(sch, mPlatform, mContext, mAllocator,
            &mCommands, mStagePool, nullptr, flags, VkExtent2D{width, height});
    swapChain.inc();
}
...
void VulkanDriver::createDescriptorSetLayoutR(Handle<HwDescriptorSetLayout> dslh,
    ...
    auto layout = resource_ptr<VulkanDescriptorSetLayout>::make(dslh, info);
...
void VulkanDriver::createDescriptorSetR(Handle<HwDescriptorSet> dsh,
    ...
    fvkmemory::resource_ptr<VulkanDescriptorSetLayout> layout =
            fvkmemory::resource_ptr<VulkanDescriptorSetLayout>::cast(dslh);
...
Handle<HwVertexBufferInfo> VulkanDriver::createVertexBufferInfoS() noexcept {
    return ResourceManager::allocHandle<VulkanVertexBufferInfo>();
}

Handle<HwVertexBuffer> VulkanDriver::createVertexBufferS() noexcept {
    return ResourceManager::allocHandle<VulkanVertexBuffer>();
}

Handle<HwIndexBuffer> VulkanDriver::createIndexBufferS() noexcept {
    return ResourceManager::allocHandle<VulkanIndexBuffer>();
}

Handle<HwBufferObject> VulkanDriver::createBufferObjectS() noexcept {
    return ResourceManager::allocHandle<VulkanBufferObject>();
}

Handle<HwTexture> VulkanDriver::createTextureS() noexcept {
    return ResourceManager::allocHandle<VulkanTexture>();
}
...
Handle<HwRenderPrimitive> VulkanDriver::createRenderPrimitiveS() noexcept {
    return ResourceManager::allocHandle<VulkanRenderPrimitive>();
}

Handle<HwProgram> VulkanDriver::createProgramS() noexcept {
    return ResourceManager::allocHandle<VulkanProgram>();
}

Handle<HwRenderTarget> VulkanDriver::createDefaultRenderTargetS() noexcept {
    return ResourceManager::allocHandle<VulkanRenderTarget>();
}
...
Handle<HwFence> VulkanDriver::createFenceS() noexcept {
    auto handle = ResourceManager::allocHandle<VulkanFence>();
    auto fence = resource_ptr<VulkanFence>::make(handle);
    fence.inc();
    return handle;
}

Handle<HwSwapChain> VulkanDriver::createSwapChainS() noexcept {
    return ResourceManager::allocHandle<VulkanSwapChain>();
}
...
Handle<HwTimerQuery> VulkanDriver::createTimerQueryS() noexcept {
    ...
    auto query = resource_ptr<VulkanTimerQuery>::construct(mTimestamps->getNextQuery());
    query.inc();
    return Handle<VulkanTimerQuery>(query.id());
}

Handle<HwDescriptorSetLayout> VulkanDriver::createDescriptorSetLayoutS() noexcept {
    return ResourceManager::allocHandle<VulkanDescriptorSetLayout>();
}

Handle<HwDescriptorSet> VulkanDriver::createDescriptorSetS() noexcept {
    return ResourceManager::allocHandle<VulkanDescriptorSet>();
}
...
void VulkanDriver::destroySwapChain(Handle<HwSwapChain> sch) {
    ...
    auto swapChain = resource_ptr<VulkanSwapChain>::cast(sch);
...
void VulkanDriver::destroyTimerQuery(Handle<HwTimerQuery> tqh) {
    ...
    auto vtq = resource_ptr<VulkanTimerQuery>::cast(tqh);
    vtq.dec();
}

void VulkanDriver::destroyDescriptorSetLayout(Handle<HwDescriptorSetLayout> dslh) {
    auto layout = fvkmemory::resource_ptr<VulkanDescriptorSetLayout>::cast(dslh);
    layout.dec();
}
...
void VulkanDriver::generateMipmaps(Handle<HwTexture> th) {
    auto t = resource_ptr<VulkanTexture>::cast(th);
    assert_invariant(t);
...
void VulkanDriver::beginRenderPass(Handle<HwRenderTarget> rth, const RenderPassParams& params) {
    ...
    for (int i = 0; i < MRT::MAX_SUPPORTED_RENDER_TARGET_COUNT; i++) {
        ...
        if (info.texture) {
            ...
            if (rpkey.samples > 1) {
                resource_ptr<VulkanTexture> sidecar = info.texture->getSidecar();
    ...
    for (int i = 0; i < MRT::MAX_SUPPORTED_RENDER_TARGET_COUNT; i++) {
        ...
        if (fbkey.samples == 1) {
            ...
            resource_ptr<VulkanTexture> texture = attachment.texture;
            if (texture->samples == 1) {
                auto const& range = attachment.getSubresourceRange();
                attachment.texture->transitionLayout(&commands, range,
                        VulkanLayout::COLOR_ATTACHMENT);
                renderPassAttachments.insert(attachment);
...
void VulkanDriver::makeCurrent(Handle<HwSwapChain> drawSch, Handle<HwSwapChain> readSch) {
    ...
    fvkmemory::resource_ptr<VulkanSwapChain> swapChain =
            fvkmemory::resource_ptr<VulkanSwapChain>::cast(drawSch);
...
void VulkanDriver::readPixels(Handle<HwRenderTarget> src, uint32_t x, uint32_t y, uint32_t width,
    ...
    auto srcTarget = resource_ptr<VulkanRenderTarget>::cast(src);
    mCommands.flush();
    mReadPixels.run(
...
void VulkanDriver::resolve(
    ...
    auto srcTexture = resource_ptr<VulkanTexture>::cast(src);
    auto dstTexture = resource_ptr<VulkanTexture>::cast(dst);
...
void VulkanDriver::blitDEPRECATED(TargetBufferFlags buffers,
    ...
    auto dstTarget = resource_ptr<VulkanRenderTarget>::cast(dst);
    auto srcTarget = resource_ptr<VulkanRenderTarget>::cast(src);
...
void VulkanDriver::bindPipeline(PipelineState const& pipelineState) {
    ...
    VulkanCommandBuffer* commands = &mCommands.get();
    auto vbi = resource_ptr<VulkanVertexBufferInfo>::cast(pipelineState.vertexBufferInfo);
    ...
    auto program = resource_ptr<VulkanProgram>::cast(programHandle);
    ...
    std::transform(setLayouts.begin(), setLayouts.end(), layoutList.begin(),
            [&](Handle<HwDescriptorSetLayout> handle) -> VkDescriptorSetLayout {
                ...
                auto layout = resource_ptr<VulkanDescriptorSetLayout>::cast(handle);
                layoutCount++;
...
void VulkanDriver::bindRenderPrimitive(Handle<HwRenderPrimitive> rph) {
    ...
    VulkanCommandBuffer* commands = &mCommands.get();
    VkCommandBuffer cmdbuffer = commands->buffer();
    auto prim = resource_ptr<VulkanRenderPrimitive>::cast(rph);
...
void VulkanDriver::draw(PipelineState state, Handle<HwRenderPrimitive> rph,
    ...
    auto rp = resource_ptr<VulkanRenderPrimitive>::cast(rph);
...
void VulkanDriver::setDebugTag(HandleBase::HandleId handleId, utils::CString tag) {
    fvkmemory::ResourceManager::associateHandle(handleId, std::move(tag));
}
